...
from datetime import datetime
from lucky_numbers import DisplayBalls
from config import *
from collections import Counter
import random
...
class LotteryBall(QMainWindow):
    ...
    def create_control_panel(self):
        ...
        lottery_dropdown = self.create_dropdown_field(lottery_options)
        self.lottery_dropdown = lottery_dropdown
        self.lottery_dropdown.currentIndexChanged.connect(self.on_lottery_selection_changed)
        control_layout.addWidget(lottery_dropdown)
        ...
        from_date_edit.setDate(QDate.currentDate().addDays(-30))
        from_date_edit.setStyleSheet("""
            QDateEdit {
                border-radius: 10px;
                padding: 8px 12px;
            ...
            QDateEdit::down-arrow {
                #image: url(:/icons/calendar-icon.png); /* Optional: replace with your icon */
                width: 16px;
                height: 16px;
            }
        """)
        from_date_str = from_date_edit.date().toString("MM/dd/yyyy")
        ...
        """)
        to_date_str = to_date_edit.date().toString("MM/dd/yyyy")
        to_layout.addWidget(to_date_edit)

        self.fetch_recent_results(self.selected_lottery_type, self.lucky_numbers, from_date_str, to_date_str)
    ...
    def check_and_fetch_results(self):
        ...
        if from_date <= to_date:
            ...
            selected_lottery_type = self.lottery_dropdown.currentText()
            
            # Start the fetch thread to get recent results
            self.start_fetch_results_thread(selected_lottery_type, from_date_str, to_date_str)
        else:
            # Optionally, you can display a warning if the date range is invalid
    ...
    def start_fetch_results_thread(self, lottery_type, from_date, to_date):
        ...
        self.fetch_results_thread = FetchResultsThread(lottery_type, [], from_date, to_date)
    ...
    def create_main_buttons_layout(self):
        ...
        generate_button = self.create_main_buttons("GENERATE")
        generate_button.clicked.connect(self.generate_lucky_numbers)     #Connect to Generate Function
    ...
    def create_right_panel(self):
        ...
        self.tab_title_label = QLabel("Tab Title")
    ...
    def create_stacked_widget(self):
        ...
        freq_table.setColumnCount(2)  # 2 columns: Number and Frequency
        freq_table.setHorizontalHeaderLabels(["Number", "Frequency"])

        # Optionally, set some table properties like row height and column width
        freq_table.setRowCount(0)  # Start with no rows
        ...
        self.history_table_display = QTableWidget()
        self.history_table_display.setColumnCount(7)
        self.history_table_display.setHorizontalHeaderLabels(["Lotto Type", "1", "2", "3", "4", "5", "6"])
        self.history_table_display.setEditTriggers(QAbstractItemView.NoEditTriggers)
        self.history_table_display.setSelectionMode(QAbstractItemView.NoSelection)
        self.history_table_display.setStyleSheet("""
            QTableWidget {
        ...
        """)
        history_layout.addWidget(self.history_table_display)

        return self.stacked_widget
    
    def add_history(table, lottery_type, lucky_numbers, mirror_table=None):
    ...
    """# Reading the selected lottery game
    ...
    def generate_lucky_numbers(self):
        ...
        self.add_history(self.history_table, lottery_type, top_6, mirror_table=self.history_table_display)
    ...
    def save_data(self):
        ...
        if file_path:
            ...
            numbers = [str(ball.number) for ball in self.lottery_balls]
...
class ImageLotteryBall(QLabel):
    ...
    def load_ball_image(self):
        image_path = f"Assets/Icons/ball_{self.ball_index}.png"
        self.pixmap = asset_manager.load_pixmap(image_path)
